...
int lidarRotateMeasure[4];
bool measured = false;

void rotateLidarTest(){ // take rooms volume.
  if (!measured){
    // rotate to starting point - 0 degrees;
    rotateTo(0);
    Serial.println("Current bearing: ");
    Serial.println(getCurrentBearing());
      for (int i = 0; i < 4; i++){
        int currentBearing = getCurrentBearing(); // rotate 90 degrees.
        int targetBearing = (currentBearing + 90 + 360) % 360;
//      Serial.println(currentBearing);
//      Serial.println(targetBearing);
        rotateTo(targetBearing);
        lidarRotateMeasure[i] = getLidarDistance();
  }
  int oneSide = lidarRotateMeasure[0] + lidarRotateMeasure[2] + 10; // 15 is the 'blind spot' that lidar misses when rotating
  int twoSide = lidarRotateMeasure[1] + lidarRotateMeasure[3] + 10;
  Serial.println(oneSide);
  Serial.println(twoSide);
  int area = oneSide * twoSide;
  Serial.println("Area is: ");
  Serial.println(area);
  measured = true;
  }
}
