...
int pulsesPerTwenty[10];

void loop() {
  int address = 0; // where to save in memory.
  
  // Get daat from memory.
  float averageFromEEPROM;
  EEPROM.get(address, averageFromEEPROM);

   lcd.setCursor(0, 0);
  lcd.print("                ");
  lcd.setCursor(0, 0);
  lcd.print("Dist. trav.: 200");
  lcd.setCursor(0, 1);
  lcd.print("                ");
  lcd.setCursor(0, 1);
  lcd.print("P./20cm avg.: ");
  lcd.print(averageFromEEPROM);
...
//    Serial.println(detectedColor);
  ...
  for(int i = 0; i < 10; i++){
    ...
    int pulseAmount = leftPulses;
  
    // Moving lidar for 20 cm and collecting amount of pulses and store in an array for later average calculation.
    moveCar(20);

    pulsesPerTwenty[i] = leftPulses - pulseAmount;
  }

  for (int i = 0; i < sizeof(pulsesPerTwenty) / sizeof(pulsesPerTwenty[0]); i++) {
    Serial.print(pulsesPerTwenty[i]);
    Serial.print(" "); // Print a space between elements for better readability.
  }

  float averagePulsesPerTwenty = calculateAverage(pulsesPerTwenty);
  Serial.println("Average pulses per twenty cm: ");
  Serial.println(averagePulsesPerTwenty);
  ...
  lcd.setCursor(0, 1);
  lcd.print("P./20cm avg.: ");
  lcd.print(averagePulsesPerTwenty);

  // Save the average value to EEPROM
  saveToEEPROM(address, averagePulsesPerTwenty);

  
  delay(300000); // Delay for 300 seconds.
}

void saveToEEPROM(int addr, float value) {
  // Write the average value to EEPROM
  EEPROM.put(addr, value);
}

float calculateAverage(int pulseArray[]) {
  int sum = 0;

  // Iterate over the array and calculate the sum of all elements.
  for (int i = 0; i < 10; i++) {
    sum += pulseArray[i];
  }
...
void moveCar(int distance) { // distance in cm.
  ...
  unsigned long moveTime = distance * 10; // 10 milliseconds per centimeter, adjust this value based on your motor and robot configuration
  ...
  while (millis() - startTime < moveTime) {
    ...
    // If the current distance is less than or equal to the specified distance, stop moving
    if (lidarDistance <= distance) {
      break;
    }
